...
namespace DogOrCat.Droid.TensorFlowClassfication
{
    public class TensorFlowClassification : IClassification
    {
        public async void Classification(byte[] bytes)
        {
            ...
            int inputSize = (int)inferenceInterface.GraphOperation("Placeholder").Output(0).Shape().Size(1);
            ...
            var resizedMap = Bitmap.CreateScaledBitmap(bitmap, inputSize, inputSize, false).Copy(Bitmap.Config.Argb8888, false);
            ...
            for (int i = 0; i < intValues.Length; ++i)
            {
                ...
                floatValues[i * 3 + 0] = (float)(intValue & 0xFF) - 105f;
                floatValues[i * 3 + 1] = (float)((intValue >> 8) & 0xFF) - 117f;
                floatValues[i * 3 + 2] = (float)((intValue >> 16) & 0xFF) - 124f;
            }

            Operation operation = inferenceInterface.GraphOperation("loss");
